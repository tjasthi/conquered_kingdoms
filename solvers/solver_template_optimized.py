...
def solve(list_of_kingdom_names, starting_kingdom, adjacency_matrix, params=[]):
    ...
    """
    # variables to change: max_cycle, conquered_weight, kingdom_benefit, distance_weight, unconquered_benefit, unconquered_penalty
    kingdom_costs = {}
    count = 0
    ...
    cost1, closed_walk1, conquered_kingdoms1 = conquer_first(list_of_kingdom_names, starting_kingdom, adjacency_matrix, kingdom_costs)
    cost2, closed_walk2, conquered_kingdoms2 = conquer_free(list_of_kingdom_names, starting_kingdom, adjacency_matrix, kingdom_costs)
...
def conquer_first(list_of_kingdom_names, starting_kingdom, adjacency_matrix, kingdom_costs):
    ...
    while (len(covered_kingdoms) != len(list_of_kingdom_names)):
        ...
        if (current_index in covered_kingdoms):
            conquered_weight = 20
        if ((next_heuristic + conquered_weight > current_heuristic) | (current_index in conquered_kingdoms)):
...
# Forces a conquer on the first node and runs through the graph
def conquer_free(list_of_kingdom_names, starting_kingdom, adjacency_matrix, kingdom_costs):
    starting_index = list_of_kingdom_names.index(starting_kingdom)
